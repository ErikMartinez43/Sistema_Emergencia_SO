...
bool inicializar_entorno()
{
    ...
    {
        fprintf(stderr, "[AMBULANCIA] Error conectando a memoria.\n");
        return false;
    }
    semaforos = crear_conjunto_semaforos(clave, 8);
    if (semaforos.sem_id == -1)
    {
        fprintf(stderr, "[AMBULANCIA] Error conectando a semáforos.\n");
        return false;
    }
    return true;
}
...
void atender_llamada(int id_unidad, int id_llamada)
{
    ...
    for (int i = 0; i < mem->contador_llamadas; ++i) {
        if (mem->llamadas[i].id == id_llamada) {
            ...
            printf("[AMBULANCIA] Llamada médica %d finalizada.\n", id_llamada);
    ...
    // Marcar ambulancia disponible nuevamente
    for (int i = 0; i < mem->contador_unidades; ++i)
        if (mem->unidades[i].id == id_unidad)
...
{
    if (argc < 3) {
        fprintf(stderr, "[AMBULANCIA] Uso: %s <id_unidad> <id_llamada>\n", argv[0]);
        return 1;
    }
